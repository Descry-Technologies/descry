...
fn classify_shell(target: &str) -> ActionClass {
    ...
    if lowercase_tokens.first().map(String::as_str) == Some("git") {
        ...
        || command_starts_with(&lowercase_tokens, &["go", "test"])
        || command_starts_with(&lowercase_tokens, &["cargo", "check"])
        || command_starts_with(&lowercase_tokens, &["cargo", "fmt"])
        || command_starts_with(&lowercase_tokens, &["cargo", "clippy"])
        || command_starts_with(&lowercase_tokens, &["cargo", "doc"])
        || command_starts_with(&lowercase_tokens, &["jest"])
        || command_starts_with(&lowercase_tokens, &["vitest"])
        || command_starts_with(&lowercase_tokens, &["mocha"])
        || command_starts_with(&lowercase_tokens, &["rspec"])
        || command_starts_with(&lowercase_tokens, &["rubocop"])
        || command_starts_with(&lowercase_tokens, &["ruff"])
        || command_starts_with(&lowercase_tokens, &["mypy"])
        || command_starts_with(&lowercase_tokens, &["tsc"])
        || command_starts_with(&lowercase_tokens, &["eslint"])
        || command_starts_with(&lowercase_tokens, &["pylint"])
        || command_starts_with(&lowercase_tokens, &["pre-commit"])
    {
        ...
        || command_starts_with(&lowercase_tokens, &["make"])
        || command_starts_with(&lowercase_tokens, &["just"])
        || command_starts_with(&lowercase_tokens, &["mvn"])
        || command_starts_with(&lowercase_tokens, &["gradle"])
        || command_starts_with(&lowercase_tokens, &["gradlew"])
        || command_starts_with(&lowercase_tokens, &["ant"])
        || command_starts_with(&lowercase_tokens, &["bazel"])
        || command_starts_with(&lowercase_tokens, &["buck"])
    {
        ...
        || command_starts_with(&lowercase_tokens, &["pip", "install"])
        || command_starts_with(&lowercase_tokens, &["pip3", "install"])
        || command_starts_with(&lowercase_tokens, &["gem", "install"])
        || command_starts_with(&lowercase_tokens, &["go", "install"])
        || command_starts_with(&lowercase_tokens, &["apt", "install"])
        || command_starts_with(&lowercase_tokens, &["apt-get", "install"])
        || command_starts_with(&lowercase_tokens, &["brew", "install"])
        || command_starts_with(&lowercase_tokens, &["brew", "upgrade"])
    {
    ...
    {
        ActionClass::ShellDelete
    } else if command_starts_with(&lowercase_tokens, &["ls"])
        || command_starts_with(&lowercase_tokens, &["cat"])
        || command_starts_with(&lowercase_tokens, &["head"])
        || command_starts_with(&lowercase_tokens, &["tail"])
        || command_starts_with(&lowercase_tokens, &["grep"])
        || command_starts_with(&lowercase_tokens, &["rg"])
        || command_starts_with(&lowercase_tokens, &["find"])
        || command_starts_with(&lowercase_tokens, &["ps"])
        || command_starts_with(&lowercase_tokens, &["env"])
        || command_starts_with(&lowercase_tokens, &["printenv"])
        || command_starts_with(&lowercase_tokens, &["which"])
        || command_starts_with(&lowercase_tokens, &["pwd"])
        || command_starts_with(&lowercase_tokens, &["whoami"])
        || command_starts_with(&lowercase_tokens, &["echo"])
        || command_starts_with(&lowercase_tokens, &["wc"])
        || command_starts_with(&lowercase_tokens, &["sort"])
        || command_starts_with(&lowercase_tokens, &["uniq"])
        || command_starts_with(&lowercase_tokens, &["date"])
        || command_starts_with(&lowercase_tokens, &["jq"])
    {
        // curl/wget are excluded: they can make destructive HTTP calls (-X DELETE/PUT/POST)
        // and are better left as Unknown so blast_radius drives the signal.
...
fn classify_cloud_delete(tokens: &[String], lowercase_target: &str) -> bool {
    command_starts_with(tokens, &["railway", "volume", "delete"])
        ...
        || command_starts_with(tokens, &["kubectl", "delete"])
        || command_starts_with(tokens, &["helm", "uninstall"])
        || command_starts_with(tokens, &["helm", "delete"])
        || command_starts_with(tokens, &["pulumi", "destroy"])
        || command_starts_with(tokens, &["terraform", "destroy"])
        || command_starts_with(tokens, &["heroku", "apps:destroy"])
        || command_starts_with(tokens, &["docker", "rmi"])
        || command_starts_with(tokens, &["docker", "rm"])
        || command_starts_with(tokens, &["docker", "system", "prune"])
        || command_starts_with(tokens, &["docker", "volume", "rm"])
    ...
    if acp.action.argument_keys.iter().any(|key| {
        ...
        (key.contains("confirm") || key.contains("force") || key.contains("danger"))
            && (key.contains("delete")
                ...
                || key.contains("drop")
                || key.contains("purge")
                || contains_word(&key, "remove"))
    }) || text.contains("delete")
        || text.contains("destroy")
        || text.contains("drop")
        || text.contains("purge")
        || contains_word(&text, "remove")
    {
        ActionClass::McpDestroy
